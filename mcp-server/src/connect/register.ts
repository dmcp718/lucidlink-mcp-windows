/**
 * Connect-API tool registrations for the unified lucidlink-api MCP server.
 *
 * Both Admin and Connect surfaces talk to the same LucidLink API container
 * (lucidlink/lucidlink-api on :3003). Keeping them as a single MCP server
 * avoids the artificial split that customers used to see in their IDE config.
 *
 * registerConnectTools() takes the McpServer instance plus shared dependencies
 * (lazy ApiClient + connectivity check), so the parent server file owns the
 * client/connectivity boilerplate exactly once.
 */
...
import { generateConnectUI } from "../blueprints/connect-ui.js";
...
export function registerConnectTools(server: McpServer, deps: ConnectToolDeps): void {
  const { getClient, ensureReady } = deps;
